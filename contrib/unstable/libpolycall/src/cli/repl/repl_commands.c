...
// Command to execute a REPL command directly
static int cmd_repl_exec(void* container, int argc, char** argv, void* context) {
    ...
    // Get REPL context
    polycall_repl_context_t* repl_ctx = cli->resolve_service(cli, "repl_context");
    if (!repl_ctx) {
        printf("Error: REPL context not initialized. Run 'repl init' first.\n");
        return 1;
    }
    
    // Check arguments
    if (argc < 1) {
        printf("Usage: repl exec <command> [args...]\n");
        return 1;
    }
    ...
    polycall_repl_status_t status = polycall_repl_execute_command(
        core_ctx, repl_ctx, command, output, sizeof(output));
    ...
    return (status == POLYCALL_REPL_SUCCESS) ? 0 : 1;
}

// Command to run REPL script
static int cmd_repl_script(void* container, int argc, char** argv, void* context) {
    polycall_cli_container_t* cli = (polycall_cli_container_t*)container;
    ...
    // Check arguments
    if (argc < 1) {
        printf("Usage: repl script <script_path>\n");
        return 1;
    }
    
    // Execute script
    polycall_core_error_t result = polycall_repl_execute_script(
        core_ctx, repl_ctx, argv[0]);
...
// Command to initialize REPL
static int cmd_repl_init(void* container, int argc, char** argv, void* context) {
    polycall_cli_container_t* cli = (polycall_cli_container_t*)container;
    ...
    // Check if REPL is already initialized
    polycall_repl_context_t* existing_repl = cli->resolve_service(cli, "repl_context");
    if (existing_repl) {
        printf("REPL already initialized. Use 'repl cleanup' before re-initializing.\n");
        return 1;
    }
    ...
    polycall_repl_config_t config = {
        ...
        .config_ctx = cli->resolve_service(cli, "config_context"),
        .output_width = 80,
        .color_output = true,
        .verbose = false
    };
    ...
    polycall_core_error_t result = polycall_repl_init(core_ctx, &repl_ctx, &config);
    if (result != POLYCALL_CORE_SUCCESS) {
        printf("Error initializing REPL: %d\n", result);
        return 1;
    }
    
    // Register in container
    cli->register_service(cli, "repl_context", repl_ctx);
    
    printf("REPL initialized successfully\n");
    return 0;
}
// Command to cleanup REPL
static int cmd_repl_cleanup(void* container, int argc, char** argv, void* context) {
    polycall_cli_container_t* cli = (polycall_cli_container_t*)container;
    ...
    polycall_repl_context_t* repl_ctx = cli->resolve_service(cli, "repl_context");
    if (!repl_ctx) {
        printf("REPL context not initialized\n");
        return 0;
    }
    ...
    cli->register_service(cli, "repl_context", NULL);
    
    printf("REPL cleaned up successfully\n");
    return 0;
}

// Command to register a custom REPL command handler
static int cmd_repl_register_handler(void* container, int argc, char** argv, void* context) {
    printf("Error: Custom handler registration can only be done programmatically\n");
    return 1;
}

// Register REPL commands
void polycall_register_repl_commands(void* registry) {
    if (!registry) {
        return;
    }
    
    // Define commands
    static const polycall_command_t interactive_cmd = {
        .execute = cmd_repl_interactive,
        .name = "interactive",
        .description = "Start interactive REPL session",
        .usage = "repl interactive [--no-color] [--verbose] [--history-file <file>] [--no-history]",
        .dependencies = (const char*[]){"core_context", NULL}
    };
    
    static const polycall_command_t exec_cmd = {
        .execute = cmd_repl_exec,
        .name = "exec",
        .description = "Execute a REPL command",
        .usage = "repl exec <command> [args...]",
        .dependencies = (const char*[]){"core_context", "repl_context", NULL}
    };
    
    static const polycall_command_t script_cmd = {
        .execute = cmd_repl_script,
        .name = "script",
        .description = "Execute a REPL script file",
        .usage = "repl script <script_path>",
        .dependencies = (const char*[]){"core_context", "repl_context", NULL}
    };
    
    static const polycall_command_t init_cmd = {
        .execute = cmd_repl_init,
        .name = "init",
        .description = "Initialize REPL context",
        .usage = "repl init [--no-color] [--verbose] [--history-file <file>] [--no-history]",
        .dependencies = (const char*[]){"core_context", NULL}
    };
    
    static const polycall_command_t cleanup_cmd = {
        .execute = cmd_repl_cleanup,
        .name = "cleanup",
        .description = "Cleanup REPL context",
        .usage = "repl cleanup",
        .dependencies = (const char*[]){"core_context", NULL}
    };
    
    static const polycall_command_t register_handler_cmd = {
        .execute = cmd_repl_register_handler,
        .name = "register-handler",
        .description = "Register a custom REPL command handler (programmatic only)",
        .usage = "repl register-handler",
        .dependencies = (const char*[]){"core_context", "repl_context", NULL}
    };
    
    // Register commands
    polycall_command_registry_register(registry, "repl", &interactive_cmd);
    polycall_command_registry_register(registry, "repl", &exec_cmd);
    polycall_command_registry_register(registry, "repl", &script_cmd);
    polycall_command_registry_register(registry, "repl", &init_cmd);
    polycall_command_registry_register(registry, "repl", &cleanup_cmd);
    polycall_command_registry_register(registry, "repl", &register_handler_cmd);
}
// Command to start interactive REPL
static int cmd_repl_interactive(void* container, int argc, char** argv, void* context) {
    polycall_cli_container_t* cli = (polycall_cli_container_t*)container;
    ...
    // Get REPL context, initializing if necessary
    polycall_repl_context_t* repl_ctx = cli->resolve_service(cli, "repl_context");
    if (!repl_ctx) {
        printf("Initializing REPL context...\n");
    ...
    polycall_core_error_t result = polycall_repl_run_interactive(core_ctx, repl_ctx);
    
    if (result != POLYCALL_CORE_SUCCESS) {
        printf("REPL execution error: %d\n", result);
        return 1;
    }
