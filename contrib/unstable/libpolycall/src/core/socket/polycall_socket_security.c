 ...
 * authentication, authorization, and integration with LibPolyCall's auth system.
 */
...
static bool validate_security_context(
    polycall_socket_security_context_t* ctx
) {
...
polycall_core_error_t polycall_socket_security_init(
    ...
    polycall_socket_security_context_t* new_ctx = 
        polycall_core_malloc(core_ctx, sizeof(polycall_socket_security_context_t));
...
polycall_core_error_t polycall_socket_authenticate(
    ...
    polycall_core_error_t error = polycall_auth_validate_token_ex(
        security_ctx->core_ctx,
        security_ctx->auth_ctx,
        token,
        &validation_result
    );
    ...
    if (!validation_result || !validation_result->is_valid) {
        ...
        if (validation_result) {
            ...
            polycall_auth_free_token_validation_result(
                security_ctx->core_ctx,
                validation_result
            );
        }
    ...
    if (validation_result->claims) {
        ...
        // Extract scopes
        if (validation_result->claims->scopes && validation_result->claims->scope_count > 0) {
        ...
        // Extract roles
        if (validation_result->claims->roles && validation_result->claims->role_count > 0) {
...
bool polycall_socket_is_authorized(
    ...
    if (!validate_security_context(security_ctx) || !auth_result || !required_scope) {
        return false;
    }
...
bool polycall_socket_has_role(
    ...
    if (!validate_security_context(security_ctx) || !auth_result || !required_role) {
        return false;
    }
...
polycall_core_error_t polycall_socket_generate_challenge(
    ...
    if (!validate_security_context(security_ctx) || !challenge_buffer || buffer_size < 32) {
        return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
    }
...
polycall_core_error_t polycall_socket_verify_challenge(
    ...
    if (!validate_security_context(security_ctx) || !challenge || !response || !token || !valid) {
        return POLYCALL_CORE_ERROR_INVALID_PARAMETERS;
    }
    ...
    *valid = false;
    ...
    // 3. Check that the response matches the expected format (e.g., HMAC of challenge)
...
bool polycall_socket_token_needs_refresh(
    ...
    uint64_t refresh_threshold = auth_result->token_expiry - security_ctx->config.token_refresh_interval;
