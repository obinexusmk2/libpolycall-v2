...
/*
 * Local credit_score_t and micro_isolation_t definitions.
 * These override the simpler stubs in types.h with the full
 * banking-specific fields needed by this module.
 */
#define credit_score_t  banking_credit_score_t
#define micro_isolation_t banking_micro_isolation_t

typedef struct banking_credit_score {
    uint32_t score;
    card_level_t card_level;
    bool gamification_unlocked;
    double credit_limit;
} banking_credit_score_t;

// Microservice isolation context
typedef struct banking_micro_isolation {
    poly_bridge_t* bridge;
    service_contract_t* contract;
    pthread_t worker_threads[4];
    bool isolated;
    void* nlm_atlas_mapping;
} banking_micro_isolation_t;
...
credit_score_t* polycall_credit_score(const char* user_id, uint32_t base_score);
