...
This script targets and fixes problematic include path patterns that cause build failures,
with an aggressive mode option to handle additional edge cases and deprecated modules.
...
class NestedPathFixer:
    """Fixes problematic include path patterns in the LibPolyCall codebase."""

    def __init__(self, project_root: str, dry_run: bool = False, 
                 verbose: bool = False, aggressive_mode: bool = False):
        ...
        self.aggressive_patterns = [
            # Handle doctor module (removed)
            (r'polycall/core/doctor/polycall_doctor\.h', r'polycall/core/polycall/polycall_core.h'),
            
            # Fix repl module references
            (r'polycall/core/repl/polycall_repl\.h', r'polycall/core/polycall/polycall_repl.h'),
            
            # Fix config_tools path issue
            (r'polycall/core/config_tools/polycall_config_tools\.h', 
             r'polycall/core/config/polycall_config_tools.h'),
             
            # Handle other potentially problematic patterns
            (r'core/core/', r'core/'),
            (r'polycall/polycall/', r'polycall/'),
            
            # Fix direct polycall header references
            (r'"polycall_([^"]+\.h)"', r'"polycall/core/polycall/polycall_\1"'),
            
            # Fix accessibility path issues
            (r'polycall/accessibility/', r'polycall/core/accessibility/'),
            
            # Fix possibly missing module prefixes
            (r'"(auth|config|network|protocol|ffi|micro|edge|telemetry)/([^"]+)"', 
             r'"polycall/core/\1/\2"'),
        ]
    ...
    def fix_file(self, file_path: Path) -> bool:
        """
        Fix problematic include paths in a single file.
        ...
        try:
            ...
            # Determine which patterns to check based on mode
            patterns_to_check = self.nested_patterns
            if self.aggressive_mode:
                patterns_to_check = self.nested_patterns + self.aggressive_patterns
            ...
            if changes_made and modified_content != original_content:
                if not self.dry_run:
                    ...
                    logger.info(f"Fixed {len(fixes)} path issues in {file_path}")
                else:
                    logger.info(f"[DRY RUN] Would fix {len(fixes)} path issues in {file_path}")
    ...
    def run(self) -> bool:
        """Run the include path fixer on all files."""
        files = self.find_all_files()
        logger.info(f"Found {len(files)} files to process")
        
        if self.aggressive_mode:
            logger.info("Running in AGGRESSIVE mode - addressing all known include path issues")

        for file_path in files:
            self.fix_file(file_path)

        # Print summary
        logger.info("\n=== Include Path Fixer Summary ===")
        logger.info(f"Mode: {'AGGRESSIVE' if self.aggressive_mode else 'Standard'}")
...
def main():
    parser = argparse.ArgumentParser(description="LibPolyCall Include Path Corrector")
    parser.add_argument("--project-root", required=True, help="Project root directory")
    parser.add_argument("--dry-run", action="store_true", help="Show what would be changed without modifying files")
    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")
    parser.add_argument("--aggressive-mode", action="store_true", 
                        help="Enable aggressive fixing of all problematic include patterns")
