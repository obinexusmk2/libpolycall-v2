/**
 * @file edge_registry.c
 * @brief Registry implementation for edge module
 */

#include <string.h>
#include <stdlib.h>
#include "polycall/core/edge/edge_registry.h"
...
edge_registry_t* edge_registry_create() {
    edge_registry_t* registry = malloc(sizeof(edge_registry_t));
    if (!registry) {
        return NULL;
    }
    
    registry->services = calloc(MAX_SERVICES, sizeof(edge_service_t));
