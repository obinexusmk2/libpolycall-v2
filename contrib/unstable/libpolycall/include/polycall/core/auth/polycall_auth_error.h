 ...
 * @return Error code
 */
polycall_core_error_t polycall_auth_error_init(
 ...
 * @param core_ctx Core context
 */
void polycall_auth_error_cleanup(
    polycall_core_context_t* core_ctx
);
 ...
 */
bool polycall_auth_error_get_last(
    polycall_core_context_t* core_ctx,
 ...
 * @return Error string
 */
const char* polycall_auth_error_to_string(
    polycall_auth_error_t error
);
...
#define POLYCALL_AUTH_ERROR_SET(ctx, hier_ctx, code, severity, message, ...) \
    polycall_auth_error_set(ctx, hier_ctx, code, severity, __FILE__, __LINE__, message, ##__VA_ARGS__)

/**
 * @brief Macro for checking auth error condition
 */
#define POLYCALL_AUTH_CHECK_ERROR(ctx, hier_ctx, expr, code, severity, message, ...) \
    do { \
        if (!(expr)) { \
            POLYCALL_AUTH_ERROR_SET(ctx, hier_ctx, code, severity, message, ##__VA_ARGS__); \
            return code; \
        } \
    } while (0)
